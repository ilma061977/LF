...
module.exports=async function(req,res){
 try{
  ...
  const features={};for(const k of featureNames){const aa=acc.map(x=>x[k]).filter(Number.isFinite),bb=win.map(x=>x[k]).filter(Number.isFinite);let weighted=0,w=0;for(const y of years){const ay=acc.filter(x=>String(x.data).endsWith(y)&&Number.isFinite(x[k])).map(x=>x[k]),wy=win.filter(x=>String(x.data).endsWith(y)&&Number.isFinite(x[k])).map(x=>x[k]);if(!ay.length||!wy.length)continue;weighted+=(mean(ay)-mean(wy))*ay.length;w+=ay.length;}const pooled=Math.sqrt(((aa.length-1)*sd(aa)**2+(bb.length-1)*sd(bb)**2)/Math.max(1,aa.length+bb.length-2));features[k]={accMean:mean(aa),winMean:mean(bb),delta:mean(aa)-mean(bb),smd:smd(aa,bb),yearAdjustedDelta:w?weighted/w:0,yearAdjustedSmd:pooled&&w?(weighted/w)/pooled:0,accSD:sd(aa),winSD:sd(bb)};}
  const numOverall=[],numStrat=[];for(let n=1;n<=25;n++){let ca=0,cw=0;for(const x of acc)if(x.dezenas.includes(n))ca++;for(const x of win)if(x.dezenas.includes(n))cw++;numOverall.push({n,accPct:ca/acc.length*100,winPct:cw/win.length*100,delta:ca/acc.length*100-cw/win.length*100});let weighted=0,w=0;for(const y of years){const ay=acc.filter(x=>String(x.data).endsWith(y)),wy=win.filter(x=>String(x.data).endsWith(y));if(!ay.length||!wy.length)continue;const ap=ay.filter(x=>x.dezenas.includes(n)).length/ay.length,wp=wy.filter(x=>x.dezenas.includes(n)).length/wy.length;weighted+=(ap-wp)*ay.length;w+=ay.length;}numStrat.push({n,yearAdjustedDelta:w?weighted/w*100:0});}
  const ids=acc.map(x=>x.concurso),runs=streaks(ids);
  const recentWindows={};for(const w of [50,100,200,500]){const tail=all.slice(-w),n=tail.filter(x=>accSet.has(x.concurso)).length;recentWindows[w]={contests:tail.length,acc:n,pct:n/Math.max(1,tail.length)*100,meanGap:n?tail.length/n:null};}
  ...
  res.setHeader('Cache-Control','no-store');res.status(200).json({ok:true,count:acc.length,allCount:all.length,
   ...
   runs:runs.slice(0,20),features,numOverall:numOverall.sort((a,b)=>Math.abs(b.delta)-Math.abs(a.delta)),numStrat:numStrat.sort((a,b)=>Math.abs(b.yearAdjustedDelta)-Math.abs(a.yearAdjustedDelta)),rateByYear,recentWindows,droughtBuckets,
