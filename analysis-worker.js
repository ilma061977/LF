...
function exhaustiveBest(ctx,policies,excluded=[],rankIndex=0,topLimit=200,progressInfo=null,quotaSpec=null,proProfile=null,virginApprovedOnly=false){
  ...
  const diag={colorPreRejected:0,patternRejected:0,colorRuleRejected:0,lineRepeatRejected:0,lineColumnRejected:0,quotaRejected:0,filterFirst:{},filterAny:{}};
  ...
  eachComb(pool,15,gref=>{const g=[...gref];tested++;
    if(!indicatedColorValid(g)){diag.colorPreRejected++;}
      ...
      if(!Number.isFinite(score)){
        ...
        if(report?.colorRule?.blocked)diag.colorRuleRejected++;if(report?.lineRepeat?.blocked)diag.lineRepeatRejected++;if(report?.lineColumnRepeat?.blocked)diag.lineColumnRejected++;
