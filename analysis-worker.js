...
function exhaustiveBest(ctx,policies,excluded=[],rankIndex=0,topLimit=200,progressInfo=null,quotaSpec=null){
  const block=new Set((excluded||[]).map(Number)),pool=ALL.filter(n=>!block.has(n));if(pool.length<15)return{game:null,score:-Infinity,tested:0,total:0,approvedCount:0,rankIndex,topGames:[],topScores:[]};
  const total=M.nCk(pool.length,15),limit=Math.max(rankIndex+1,Math.min(1000,Number(topLimit)||200)),top=[];let tested=0,approvedCount=0,lastProgress=0;const progressEvery=Math.max(1000,Math.floor(total/250));
  eachComb(pool,15,gref=>{const g=[...gref];tested++;if(indicatedColorValid(g)&&quotaAllows(g,quotaSpec)){const score=M.candidateScore(g,ctx,policies);if(Number.isFinite(score)){approvedCount++;insertExhaustiveTop(top,{game:g,score},limit);}}if(tested-lastProgress>=progressEvery||tested===total){lastProgress=tested;const provisional=top[Math.min(rankIndex,Math.max(0,top.length-1))]||top[0]||null;if(progressInfo?.type==='backtest')postMessage({type:'backtest-integral-progress',contest:progressInfo.contest,current:progressInfo.current,total:progressInfo.targets,comboTested:tested,comboTotal:total,approvedCount});else if(progressInfo?.type==='combined-integral')postMessage({type:'combined-integral-combo-progress',contest:progressInfo.contest,current:progressInfo.current,total:progressInfo.targets,comboTested:tested,comboTotal:total,approvedCount});else postMessage({type:'progress',tested,total,maxAttempts:total,found:approvedCount,approvedCount,provisionalGame:provisional?.game||null,provisionalScore:provisional?.score??null,mode:'Busca exaustiva integral · 51 filtros · F28 + F29 + F36 obrigatórios'});}});
  const picked=top[rankIndex]||null;return{game:picked?.game||null,score:picked?.score??-Infinity,tested,total,approvedCount,rankIndex,topGames:top.map(x=>x.game),topScores:top.map(x=>x.score)};
}
...
function runBacktest(d){
  ...
  const targets=history.slice(-Math.min(w,Math.max(0,history.length-10))),strat=[],randomSeries=Array.from({length:series},()=>[]),blockPass=[];let eligible=0,skipped=0;
  ...
  for(let i=0;i<targets.length;i++){
    ...
    const best=mode==='integral'?exhaustiveBest(ctx,policies,[],0,1,{type:'backtest',contest:target.concurso,current:i+1,targets:targets.length},quota):deterministicBestFromPool(ctx,policies,sampledGames(sampleSize).filter(g=>quotaAllows(g,quota)));const gen=best.game;
